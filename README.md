...
Jenkins가 `Checkout → 테스트 → JAR 빌드 → 운영 서버 전송 → systemd 재시작` 파이프라인을 자동 수행한다. 운영 서버는 Docker 없이 `java -jar`를 systemd(`stylehub.service`)로 직접 구동하는 네이티브 배포 방식이다.

- **로그 영속화**: prod 환경에서는 콘솔(systemd journal)에 더해 파일로도 로그를 남긴다. 일 단위 회전, 14일 보관, 전체 용량 1GB 상한.
- **메트릭 노출**: Actuator에 `/actuator/prometheus`를 열어 JVM·HTTP·DB 커넥션 풀 지표를 노출한다. 수집기(Prometheus)와 대시보드 연동은 다음 단계다.
- **배포 실패 알림**: Jenkins 파이프라인이 실패하면 Slack으로 알림이 간다.
- **로컬 개발 환경**: `docker-compose.yml`로 MySQL·Redis만 띄워 로컬에 직접 설치하지 않고도 개발할 수 있다. 운영 배포에는 사용하지 않는다.
