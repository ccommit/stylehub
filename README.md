...
“**동시에 10만 명이 몰려도, 데이터는 절대 틀리지 않는다.**”
 ...
 > 대용량 트래픽 처리, 데이터 정합성 보장, 동시성 제어 전략, 테스트 코드 기반 검증, CI/CD 자동화를 통해 실제 운영 환경을 가정한 시스템 안정성을 구현했습니다.
 

 > 대용량 트래픽 환경에서 발생하는 동시성 문제와 데이터 정합성 이슈를 직접 설계로 해결해보고자 본 프로젝트를 기획했습니다.
