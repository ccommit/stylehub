...
2. 관리자로 로그인해 입점을 승인한다: `PATCH /api/v1/admin/stores/{storeId}/approve`. 관리자 이메일과 비밀번호는 실행할 때 사용자가 환경변수로 넣는다.
3. 승인된 스토어로 로그인해 상품을 스토어마다 40개(옵션 포함) 등록한다: `POST /api/v1/stores/{storeId}/products`
